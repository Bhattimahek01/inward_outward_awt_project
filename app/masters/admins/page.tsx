...
export default function AdminMaster() {
        ...
        const fetchRoles = async () => {
            try {
                ...
                if (data.length > 0 && !formData.RoleID) {
                    setFormData(prev => ({ ...prev, RoleID: data[0].RoleID.toString() }));
                }
            } catch (err) {
                console.error(err);
            }
    ...
    const filteredUsers = users.filter(user => {
        ...
        const matchesFilter = activeFilter === "All" || user.role.RoleName.toLowerCase() === activeFilter.toLowerCase();
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading || isRefreshing ? (
                                ...
                                <tr key={user.UserID} className={`hover:bg-slate-50/30 transition-colors group ${selectedIds.includes(user.UserID) ? 'bg-blue-50/30' : ''}`}>
                                    ...
                                    <td className="px-8 py-6">
                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold ${user.role.RoleName === 'admin'
